...
public class WorldToUI : MonoBehaviour
{
    ...
    public void JumpToUIPriv(Transform currentObject,RectTransform targetUI)
    {
        ...
        currentObject.DOScale(currentObject.lossyScale + new Vector3((currentObject.lossyScale.x * 30) / 100, (currentObject.lossyScale.y * 30) / 100, (currentObject.lossyScale.z * 30) / 100), .2f).SetEase(Ease.OutQuad).OnComplete(() => currentObject.GetComponent<TapAndCollectItems>().PopUp());
